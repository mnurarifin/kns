...
<script>
    $(function() {
        ...
        getTransactionPayment = () => {
            $("#table").dataTableLib({
                url: window.location.origin + "/member/stockist/get-transaction-history/",
                ...
                colModel: [{
                    ...
                    align: "center",
                    render: (params, data) => {
                        return `<a onclick="showModalDetail(${params})" title="Detail" data-toggle="tooltip" class="cstmHover"> <i class="bx bx-book info"></i> </a>`
                    },
                }, {
                    display: "Tanggal",
                    ...
                    align: "left",
                    render: (params, args) => {
                        if (args.warehouse_transaction_delivery_method == 'pickup') {
                            return `<span>Kode Transaksi : ${args.warehouse_transaction_code}</span>`
                        } else {
                            return `<span>Kode Transaksi : ${args.warehouse_transaction_code}</span><br>
                            <span>Ekspedisi : ${args.warehouse_transaction_delivery_courier_code} - ${args.warehouse_transaction_delivery_courier_service}</span>`
                        }
                    }
                }, {
                    display: "Status",
                    ...
                    render: (params) => {
                        switch (params) {
                            ...
                            case 'paid':
                                return `<span class="badge badge-info rounded-pill">Menunggu Dikirim Admin</span>`
                                break;

                            case 'delivered':
                                return `<span class="badge badge-info rounded-pill">Dikirim Oleh Admin</span>`
                                break;
        ...
        showModalDetail = (id) => {
            ...
            <tr class="">
                <th class="text-left" colspan="3">Jumlah Pembayaran</th>
            ...
            $("#table-detail").removeClass('d-none')
            $("#table-detail-seller").addClass('d-none')
