<?php
namespace App\Libraries\Securimage;
...
class Securimage {
    ...
    */
     ...
     * @var int
     */
    private $iscale; // internal scale factor for anti-alias @hkcaptcha
    ...
    function show($background_image = "")
    {
        if($background_image != "" && is_readable($background_image)) {
            ...
            $this->bgimg = __DIR__ . DIRECTORY_SEPARATOR . 'bg' . DIRECTORY_SEPARATOR . '1.png';
        }
    ...
    function drawWord()
    {
        ...
        if ($this->use_gd_font == true || !is_readable($this->ttf_file)) {
            // print_r($this->gd_font_file);exit;
            if (!is_int($this->gd_font_file)) { //is a file name
                $font = imageloadfont(__DIR__ . DIRECTORY_SEPARATOR . $this->gd_font_file);
                if ($font == false) {
