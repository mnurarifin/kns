$.fn.dataTableLib = function (property) {
  ...
  $(document).on(
    ...
    (e) => {
      ...
      if ($(e.currentTarget).hasClass("class-selected")) {
        ...
        $("." + $(tableID).attr("id") + "-selected-" + property.selectID).prop(
          "checked",
          false
        );
    ...
    if ($(e.currentTarget).hasClass("class-selected")) {
      ...
      if (property.select == true && property.multiSelect == true) {
        ...
        if (countSelectedTD < countTD) {
          ...
          $("." + $(tableID).attr("id") + "-select-all-th").removeClass(
            "class-selected"
          );
          $(e.currentTarget)
            .closest("tr")
  ...
  $(document).on("click", `.${$(tableID).attr("id")}-sort`, function () {
    let dtTableLimit = $("." + $(tableID).attr("id") + "-dtTable-limit").val();
    let dataSort =
      $(this).attr("data-sort") == undefined || $(this).attr("data-sort") == ""
        ? "asc"
        : $(this).attr("data-sort");
...
$.setTable = function (t, property, dtTablePage) {
  ...
  $.each(property.buttonAction, function (i, value) {
    actionButton =
      actionButton +
      "<button class='btn btn-icon btn-outline-primary btn-sm mb-1 dtable-action " +
      $(t).attr("id") +
      "-action " +
      ...
      "' data-message='" +
      value.message +
      "' style='padding-top:0px;padding-bottom:0px;display:flex;align-items:center;'><i class='ficon " +
      (value.icon == undefined ? "" : value.icon) +
      "' style='top:0px;'></i><span style='color:#000'>" +
      value.display +
      "</span></button>";
  });
  searchButton = `<div class="row"><div class="col-sm-12 col-md-6" id="${$(
    t
        ...
        </div><div class="col-sm-12 col-md-6 d-flex justify-content-end">
        ${actionButton}
        </div></div>`;
  ...
  if (typeof property.search !== "undefined" && property.search == true) {
    ...
    if (typeof property.searchItems !== "undefined") {
      $.each(property.searchItems, function (key, value) {
        ...
        if (value.type == "text") {
          modalSearchBody =
            modalSearchBody + "<label>" + value.display + "</label>";
          modalSearchBody =
            modalSearchBody +
          ...
          modalSearchBody =
            modalSearchBody +
            '<div class="col-md-6"><label>Start</label><input type="date" name="' +
            value.name +
          ...
          modalSearchBody =
            modalSearchBody +
            '<div class="col-md-6"><label>End</label><input type="date" name="' +
            value.name +
    ...
    searchButton = `<div class="row"><div class="col-sm-12 col-md-6 col-lg-4" id="${$(
      t
    ).attr("id")}-search-result">
          </div><div class="col-sm-12 col-md-6 col-lg-8 d-flex justify-content-end">
          ${actionButton}
          <button class="btn btn-icon btn-outline-primary mb-1" data-toggle="modal" data-target="#${$(
            t
          ).attr(
            "id"
          )}-searchModal" style='padding-top:4px;'><i class="ficon bx bx-search"></i><span style="font-size:13px;color:#000;">Cari</span></button>
          </div></div>`;
    modalSearch =
      modalSearch +
      '<div class="modal" id="' +
    ...
    modalSearch =
      modalSearch +
      `<button type="button" class="close" onclick="closeModal('#${$(t).attr(
        "id"
      )}-searchModal')"  >&times;</button>`;
    modalSearch = modalSearch + "</div>";
    modalSearch =
      modalSearch +
      '<form id="' +
      $(t).attr("id") +
      '-dtTable-form"><div class="modal-body" style="overflow: scroll;">' +
      modalSearchBody +
      "</div>";
    ...
    modalSearch =
      ...
      `<button type="button" class="btn btn-light-secondary" onclick="closeModal('#${$(
        ...
        "id"
      )}-searchModal')" style="border:margin-right:2px;">Tutup</button><button type="submit" class="btn btn-primary" style="margin-left:2px;">Cari</button>`;
    ...
    if (value.sortAble == true) {
      ...
      if (value.name == property.sortName) {
        if (property.sortOrder == "desc") {
          sortStatus = `<i class="text-primary bx bx-sort-z-a ${$(t).attr(
            "id"
          )}-sort" data-field="${value.name}" data-sort="asc"></i>`;
        } else {
          sortStatus = `<i class="text-primary bx bx-sort-a-z ${$(t).attr(
            "id"
          )}-sort" data-field="${value.name}" data-sort="desc"></i>`;
        }
      } else {
  ...
  dtTable =
    '<table class="table table-bordered">' + tmpdtThead + dtTbody + "</table>";
  $(t).html(dtTable);
  $(`.${$(t).attr("id")}-tbody`).html(
    `<td style="text-align:center;" colspan="${dtOrderHead.length}"><img src="/admin-assets/images/icon/loading.gif" width="20"></td>`
  );
  ...
  if (property.select == true && property.multiSelect == true) {
    ...
    dtTable = `<form id="${$(t).attr(
      "id"
    )}-dtTable-form-select"><table class="cstm-table table table-bordered">${dtThead}${dtTbody}</table></form>`;
    ...
    dtTable =
      '<table class="cstm-table table table-bordered">' +
      dtThead +
      dtTbody +
      "</table>";
  }
  ...
  if (property.tableIsResponsive) {
    dtTable =
      searchButton + '<div class="table-responsive">' + dtTable + "</div>";
  } else {
...
$.getData = function (
  ...
  var advancedOption = [];
  $.each(property.advancedOption, function (key, value) {
    if (value) {
      advancedOption.push(value);
    }
  });
  ...
  $.ajax({
    ...
    success: function (response) {
      if (advancedOption.length > 0) {
        $.each(advancedOption, function (key, value) {
          $("#" + value.id).text(response.data[value.value]);
        });
      }
      if (isSearch < 1 && response.data.pagination.total_data == 0) {
        ...
        tbody =
          ...
          (property.select == true
            ? parseInt(dtOrderHead.length + 1)
            : dtOrderHead.length) +
          '" class="text-center">Data tidak ditemukan</td></tr>';
      } else {
          ...
          $.each(dtOrderHead, function (key, field) {
            ...
            if (field.action !== undefined) {
              if (field.action.function !== undefined) {
                let data = JSON.stringify(value)
                  .replace(/\'/g, "\\'")
                  .replace(/\"/g, "'");
                // text = `<a href="javascript:;" onclick="${field.action.function}(${data})" class="btn btn-${field.action.class} btn-icon btn-sm"><i class="${field.action.icon}"></i><a>`
              ...
              if (field.action.url !== undefined) {
                // text = `<a href="${field.action.url}" class="btn btn-${field.action.class} btn-icon btn-sm"><i class="${field.action.icon}"></i><a>`
            ...
            tbody =
              ...
              (property.colModel[irWidth].width == undefined
                ? ""
                : "max-width: " + property.colModel[irWidth].width) +
              ';">' +
              text +
              "</td>";
            irWidth++;
...
$.setLimit = function (t, limitOptions, currentLimit) {
  let limit = '<div class="col-sm-12 col-md-6">';
  limit = limit + '<div class="d-flex"><fieldset class="form-group">';
  limit =
    limit +
    '<select class="form-control ' +
    $(t).attr("id") +
    '-dtTable-limit" style="width: 70px; height: 34px; background-position-y: center;" id="dtTable-limit">';
  ...
  limit =
    limit +
    '<span style="margin-left: 30px!important;font-size: 12px;height: 34px;display: flex;align-items: center;" class="' +
    $(t).attr("id") +
    '-count-data"></span></div>';
  limit = limit + "</div>";
  return limit;
...
$.setPagination = function (property, data) {
  ...
  pagination =
    pagination +
    '<ul class="pagination pagination-success justify-content-end">';
  if (data.prev != "0") {
    pagination =
      pagination +
...
$.updateTable = function (
  ...
  $(`.${$(t).attr("id")}-tbody`).html(
    `<td style="text-align:center;" colspan="${
      property.select == true ? dtOrderHead.length + 1 : dtOrderHead.length
    }"><img src="/admin-assets/images/icon/loading.gif"></td>`
  );
  let results = $.getData(
    t,
    property,
      ...
      if (response.status == 200) {
        ...
        if (response.data.success > 0 && response.data.failed == 0) {
          ...
          $message =
            $message +
            "<br>Success : " +
            (response.data.success == undefined ? 0 : response.data.success) +
            ". Gagal : " +
            (response.data.failed == undefined ? 0 : response.data.failed);
          icon = "warning";
          $.refreshTable($(data.tableID).attr("id"));
        }
        try {
