                    ...
                    <div class="form-body">
                        <div class="row">
                            ...
                            <div class="col-md-8 form-group">
                                <input type="text" class="form-control" name="ron_title" placeholder="Nama Reward">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                                <label>Nilai Persentase (%)</label>
                            </div>
                            <div class="col-md-8 form-group">
                                <input class="form-control moneypercentage" name="ron_value" id="ron_value" placeholder="Nilai Bonus Reward (Rp)">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-8 form-group">
                                <input class="form-control money" name="ron_condition_node_left" id="ron_condition_node_left" placeholder="Syarat Poin Kiri">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-4">
                            ...
                            <div class="col-md-8 form-group">
                                <input class="form-control money" name="ron_condition_node_right" id="ron_condition_node_right" placeholder="Syarat Poin Kanan">
                            </div>
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.moneypercentage').maskMoney();
        $("#table-reward").dataTableLib({
            url: window.location.origin + '/admin/service/ron/getData/',
            selectID: 'ron_id',
            colModel: [{
                    display: 'Nama RON',
                    name: 'ron_title',
                ...
                {
                    display: 'Poin Kiri',
                    name: 'ron_condition_node_left',
                    sortAble: false,
                    align: 'center'
                },
                {
                    display: 'Poin Kanan',
                    name: 'ron_condition_node_right',
                    sortAble: false,
                    align: 'center'
                },
                {
            ...
            buttonAction: [{
                ...
                },
                {
                    ...
                    style: "success",
                    action: "active",
                    url: window.location.origin + "/admin/service/ron/activeRon"
                },
                {
                    ...
                    style: "danger",
                    action: "nonactive",
                    url: window.location.origin + "/admin/service/ron/notActiveRon"
                },
                {
                    display: 'Hapus',
                    icon: 'bx bx-trash',
                    style: "danger",
                    action: "remove",
                    url: window.location.origin + "/admin/service/ron/removeRon",
                    message: 'Hapus'
                }
            ]
        })
    ...
    $('#formAddUpdate').on('submit', (e) => {
        ...
        formData.set('ron_value', $('#ron_value').val());
        formData.set('ron_condition_node_left', $('#ron_condition_node_left').val().replace(/\./g, ""));
        formData.set('ron_condition_node_right', $('#ron_condition_node_right').val().replace(/\./g, ""));

        let url = state.baseUrl + state.addUrl;
        if (state.formAction == 'update') {
            formData.append('ron_id', state.selectID);
            formData.append('ron_old_image_filename', state.selectedOldImage);
            url = state.baseUrl + state.updateUrl;
        }
    ...
    let state = {
        ...
        addUrl: '/admin/service/ron/actAdd/',
        updateUrl: '/admin/service/ron/actUpdate/',
        selectID: '',
        selectedOldImage: ''
    }
    ...
    function addRon() {
        $('#response-message').html('')
        $('#modalAddUpdateTitle').text('Form Tambah Data Rons')
        state.selectID = '';
    ...
    function updateRon(ron) {
        state.selectID = ron.ron_id;
        state.selectedOldImage = ron.ron_image_filename;
        state.formAction = 'update';
        $('#response-message').html('');
        $('#modalAddUpdateTitle').text('Form Ubah Data Ron');


        $('#formAddUpdate input[name=ron_title]').val(ron.ron_title);
        $('#formAddUpdate input[name=ron_value]').val(ron.ron_value);
        $('#formAddUpdate input[name=ron_condition_node_left]').val(ron.ron_condition_node_left);
        $('#formAddUpdate input[name=ron_condition_node_right]').val(ron.ron_condition_node_right);
        // $('#formAddUpdate input[name=rewardImageFilename]').val(reward.reward_image_filename);
        $('#previewImage').prop('src', ron.ron_image_filename);
        $('#modalAddUpdate').modal('show');
        $(".money").maskMoney('mask');
        $('.moneypercentage').maskMoney('mask');
    }
