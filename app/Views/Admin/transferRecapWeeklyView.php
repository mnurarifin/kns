    ...
    function export_komisi(data){
        Swal.fire({
            ...
            if (result.value) {
                ...
                .append($('<input/>')
                    .attr({ 'type': 'hidden', 'name': 'columns', 'value': data.exportColumns['columns'] })
                ).append($('<input/>')
                    .attr({ 'type': 'hidden', 'name': 'display', 'value': data.exportColumns['display'] })
                ).append($('<input/>')
                    .attr({ 'type': 'hidden', 'name': 'sort', 'value': data.property.sortName })
                ).append($('<input/>')
                    .attr({ 'type': 'hidden', 'name': 'dir', 'value': data.property.sortOrder })
                )
                );
                let formData =  $('#dtexport-excel-form').serializeArray();

                $.ajax({
                    url: "<?php echo site_url('/transfer/excel_approval/weekly') ?>",
                    type: 'POST',
                    data:  formData,
                    "content_type": 'application/json',
                    success: function(res) {
                        if (res.status == 200) {
                            window.location.origin+"/transfer/approval_weekly/";
                        }
                    },
                    error: function(res) {
