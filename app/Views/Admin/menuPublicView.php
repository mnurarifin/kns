    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalAddUpdateTitle">Form Menu</h5>
...
<script type="text/javascript">
    ...
    let stateMenu = {
        ...
        addUrl: '/admin/service/menu/addMenuPublic',
        updateUrl: '/admin/service/menu/updateMenupublic',
    ...
    $(document).ready(function() {
        if (parentId == 0) {
            $('.titleHead').text('Data Menu')
            $("#table-menu").dataTableLib({
                url: window.location.origin + '/admin/service/menu/getDataMenuPublic',
                selectID: 'menu_id',
                colModel: [{
                        display: 'Urutan Menu',
                        name: 'menu_id',
                    ...
                    },
                    {
                        ...
                        action: {
                            function: 'subMenu',
                            icon: 'btn btn-icon btn-outline-info bx bxs-customize'
                        }
                    },
                    {
                        display: 'link',
                ...
                buttonAction: [{
                    ...
                    },
                    {
                        ...
                        style: "danger",
                        action: "remove",
                        url: window.location.origin + "/admin/service/menu/removeMenuPublic",
                        message: 'Oke'
                    },
                    {
                        display: 'Aktifkan',
                        ...
                        style: "success",
                        action: "active",
                        url: window.location.origin + "/admin/service/menu/activeMenuPublic"
                    },
                    {
                        ...
                        style: "warning",
                        action: "nonactive",
                        url: window.location.origin + "/admin/service/menu/nonActiveMenuPublic"
                    },
                ]
            });
        } else {
            ...
            $("#table-menu").dataTableLib({
                url: window.location.origin + '/admin/service/menu/getDataMenuPublic/' + parentId,
                selectID: 'menu_id',
                colModel: [{
                        display: 'Nama menu',
                        name: 'menu_title',
    ...
    function subMenu(submenu) {
        window.location.href = '/menu/public-menu/show?menuPar=' + submenu.menu_id + '&menutitle=' + submenu.menu_title
    }

    function backMenu() {
        window.location.href = '/menu/public-menu/show'
    }
    ...
    function updateMenu(menu) {
        ...
        stateMenu.selectedID = menu.menu_id;
        stateMenu.selectedParID = parentId;
        stateMenu.formAction = 'update';
        $('#formAddUpdate input[name=titleMenu]').val(menu.menu_title);
        $('#formAddUpdate input[name=linkMenu]').val(menu.menu_link);
        $('#modalAddUpdate').modal('show');
    }

    function detailMenu(menu) {
        $('#menu-name').text(menu.menu_title);
        $('#menu-status').text(menu.menu_is_active == '1' ? 'Aktif' : 'Tidak Aktif');
        $('#menu-content').text(menu.content_title ? menu.content_title : '-');
    ...
    function sortOrderMenu(id, op) {
        ...
        $.ajax({
            url: window.location.origin + '/admin/service/menu/sortOrderPublicMenu',
            method: "POST",
            data: {
                idMenu: id,
                orderMenu: op
